...
다행인 점은 고객이 직접 요청한 VC가 아닌 내부에서 시스템 테스트를 통해 발견 되었고 또 해당 장비를 국내에서 많이 안 쓴다는 점이 그나마 다행이었죠.
...
면밀히 분석하기 위해서 위에서 나왔던 모니터링 기능을 추가했고 이는 후에 사용하기 용이하게 설정 값으로 on/off가 되게끔 해두었습니다.
...
데이터 통신 프로세스를 하나씩 다 분석하기에는 시리얼로 통신하는 경우, Http로 통신하는 경우 등 다양한 통신 법이 있고 그걸 또 다양한 장비에서 다 각자의 방법을 사용하고 있었기에 통합적으로 성능을 향상시키면서 적은 리소스를 넣을 수 있는 건 캐싱 기능 추가가 가장 적합해보였습니다.
...
## 삽질의 결과
아쉬운 한편으로 이번 이슈를 찾기 위해 분석하면서 로깅 기능 개선하여 안정성이 올랐고 캐싱 기능이 추가되면서 시스템의 성능도 향상되었기 때문에 만족스러운 이슈였습니다.