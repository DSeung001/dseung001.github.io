...
장비 들에서 찍은 데이터를 의사들이 쉽게 보고, 관리, 수정하기 위한 일종의 의료 데이터 통합 웹 통합 툴이죠.
이 서비스에서 장비와의 데이터 통신 부터 받은 데이터에 대한 파이프라인, 프론트 작업과 퍼블리싱 작업까지 넓게 맡고 있죠. 

해당 글에서 다룰 이슈는 다음과 같습니다.<br/>
특정 장비에서 부터 일정 양 이상의 데이터를 받으면 데이터베이스가 죽어버린다는 문제가 발생합니다. 
안과 장비의 데이터를 안정적으로 수신하여 의사한테 정확한 데이터를 보여줘하는 서비스 특성상 매우 큰 이슈였고

원인을 알 수 없다는 점도 매우 위험 하였기에 해당 이슈에 우선 순위를 높게 두고 분석을 진행했습니다.
결과 적으로 마무리 및 정리까지 대략 5일 정도를 소요하여 해결 할 수 있었습니다.

이번 경험을 통해 Go의 내장 SQL 패키지와 커넥션 풀의 관계를 더 자세히 알 수 있었고 로깅 리팩토링, 캐싱 기능 추가로 프로젝트에 안정성과 성능을 챙기는 같이 챙기는 것의 중요성을 다시금 깨달았습니다. 
...
현재 사용중인 Connections의 개수가 남게끔 로그를 추가해두었습니다.
```text
...
여기를 보시면 <b>Total DB Connections: 2</b>로 커넥션 풀이 사용되자 말자 반환되었기에 수치가 늘지 않은 걸 알 수 있죠.
```text
...
## 해결 과정
### 커넥션 풀 모니터링
커넥션 풀 문제로 의심이 되었지만 관련되서 정보를 볼 수 있는 기능이 없었습니다.
문제 상황이 커넥션 풀로 의심은 되었지만 보다 면밀히 분석하기 위해서 위에서 나왔던 모니터링 기능을 추가했고 이는 후에 사용하기 용이하게 설정 값으로 on/off가 되게끔 해두었습니다.<br/>
이는 앞으로도 비슷한 상황이 생겼을 때 쉽게 확인하기 위함입니다.
...
```
커넥션 풀 모니터링 기능과 별도로 모든 DB 접근 쿼리를 로깅할 수 할 수 있도록 DB 인스턴스 로직을 사용할 때 래핑 클래스를 접근하게 해서 시스템에서 사용하는 쿼리 모든 쿼리에 대해서도 로그를 남길 수 있게 해뒀습니다. <br/>
쿼리 중에 잘못된 부분이나 특정 쿼리가 많이 실행된다면 개선이 필요할 경우 쿼리 튜닝을 진행하기 위해서죠

전체적으로 나중에 비슷한 문제가 발생하더라도 모니터링에 쓸 수 있는 재사용성을 우선순위로 두고 작업했습니다.
...
아래 해당 코드이며 <b>[]log.Level{log.DebugLevel, log.InfoLevel, log.WarnLevel, log.ErrorLevel}</b> 부분을 로깅 레벨을 받고 있지만
정작 라이브러리의 NewTimeBasedRollingFileHook에서는 이 값을 받기만하고 사용하고 있지 않았습니다.
```go
...
동일한 결과를 참조하는 쿼리 문을 장비와 데이터 통신을 할 때 40~50건 씩이나 호출한다는 점을 확인 했습니다. <br/>
이걸로 커넥션 풀이 고갈되는 현상을 100% 설명할 수 없었지만 혹시나 하는 마음에 캐싱을 추가했습니다. <br/>
데이터 통신 프로세스를 하나씩 다 분석하기에는 시리얼로 통신하는 경우, Http로 통신하는 경우 등 다양한 통신 법이 있고 그걸 또 다양한 장비에서 다 각자의 방법을 사용하고 있었기에 통합적으로 성능을 향상시키면서 적은 리소스를 넣을 수 있는 건 캐싱 기능 추가가 가장 적합해보였습니다.<br/>
온프레미스 환경이기에 설치를 하는 과정에 복잡성을 최소화 하기 위해 외부 솔루션보다는 in-memory 방식을 선택했습니다.
...
캐싱 추가로 확실히 많은 양의 DB 접근이 줄었습니다. 수치로 보면 230건이던 DB 접근이 진단에 대한 캐싱을 추가하는 걸로 180건으로 줄었습니다.<br/>
...
## 결과
결과 적으로 캐싱이 추가되어 장비와 통신시 불필요한 DB 접근을 50건을 줄여 DB 객체 접근량을 230건에서 180건으로 78%로 줄었고<br/>
