...
- Parameter Store: `/class-s/prod/ECR_REGISTRY`에 ECR 서버 주소를 주고 `/class-s/prod/IMAGE_TAG`에 마지막 성공 배포 SHA를 둡니다. EC2의 `deploy.sh`가 pull 대상을 여기서 읽고, Actions가 배포 성공 후 `IMAGE_TAG`를 갱신합니다.
...
IAM Role 세팅이 가장 까다로웠지만 AWS Q가 생각보다 잘 도와줘서 진행하기 편했습니다. 무엇보다 AWS에서 제공하는 방식이니 문서의 버전이슈나 보안 문제를 신경 덜 써도 되어 좋았죠.
