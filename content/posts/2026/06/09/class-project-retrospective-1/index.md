...
# 개요
1차 MVP로 `http://43.201.141.69/`로 배포를 진행했습니다.
...
```
위 파이프라인까지 구현해 6장 초반 화면처럼 자연어 검색이 동작합니다. <br/>
...
비교적 사이즈가 작은 규모에서는 `HLS + S3 + CF` 구조면 충분해 보였지만 욕심이 생겨서 처음 배포할 때부터 바이트 단위로 청크를 만드는 넷플릭스 방식으로 인코딩 서버까지 만들고 백엔드 서버와 통신하는 구조를 구상했지만 이에 대해서 실제 서비스의 생명주기를 간접 체험하는 걸 위해 배포 후 고도화하는 방식을 피드백으로 받았습니다.
...
실 서비스를 배포하는 게 목표지만 결국엔 진짜 목적은 동영상을 다루는 서비스를 경험하는 부분이라서 다음처럼 스프린트 달력으로 당일 할 수 있는 영역을 정해서 진행했습니다.
![sprint](./sprint.webp)

## 기능

이 프로젝트의 부모가 되는 프로젝트로 OTT 사이트 클론 코딩이 있었습니다.
그때 클론 대상으로 삼았던 사이트의 기술 스택이 `Django`, `Next.js`, `React`로 정하였습니다.
이 스택의 장점은 빠른 생산성과 안전성이기에 이를 채택했던 곳도 비슷한 이유로 정한 걸로 보였습니다.
...
- S3 staging과 워커 `tmp` 삭제
- `PublishJob` 상태를 `completed`로 변경
...
# 확장
개발 예정인 기능이 아직 많습니다.
실질적으로 사용자에게 가장 큰 변화를 주는 기능을 먼저 하면 아래 부분이 난이도 대비 효과가 클 것 같고
- 업로드가 아닌 유튜브 링크를 통한 공유
- 자유 게시판

실제로 제가 공부하고 싶은 분야에 대해서 변경이 필요합니다.
