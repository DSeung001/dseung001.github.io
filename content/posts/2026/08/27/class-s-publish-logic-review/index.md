...
아래 흐름 컨펌
# 흐름
세 기능은 따로 보이지 않습니다. 강의가 올라올 때 문제를 만들고, 그 문제를 풀고 영상을 본 기록을 랭킹으로 보여 주며, 다음날 아침 메일로 다시 한 번 꺼내 줍니다.

이전 [하이브리드 검색](../../07/class-s-hybrid-search/)에서 나눈 `publish` / `metadata` / `search_index` 잡 중, 문제는 `metadata` 잡에 붙습니다. HLS 인코딩 잡이 아니라, 자막을 뽑고 설명, 태그, 타임라인을 채우는 그 호출에 객관식 JSON을 같이 받는 구조입니다.

## 문제 발행
자막이 있어야 문제를 만듭니다. `metadata` 잡은 `transcribe` 다음에 `summarize`로 넘어가고, 여기서 Gemini가 description, tags, timelines와 함께 `problems`를 한 JSON으로 돌려줍니다.

설명, 태그, 타임라인은 검증에 실패하면 잡 자체가 실패합니다. 문제는 다릅니다. `normalize_practice_problems`가 형식을 통과하지 못하면 빈 목록을 남기고, 저장이 깨져도 잡은 완료로 끝냅니다. 카탈로그에 쓸 본문이 막히면 안 되고, 문제는 검수 화면에서 다시 만들 수 있어서입니다.
