...
처음에는 이름 때문에 역인덱스가 일반 인덱스와 같은 계열이라고 들었습니다.<br/>
자세히 보니 일반 인덱스는 원본 행을 빠르게 찾기 위해 키를 정렬해 B-tree로 탐색하는 구조이고, 역인덱스는 검색 질의에 맞춰 `토큰 → 행` 매핑을 따로 두고 그 목록으로 관련 행을 찾는 구조입니다. 둘을 완전히 다른 개념으로 나누어 보니 이해가 편해졌습니다.
...
Class S에서는 이 행이 곧 청크입니다. 영상 전체를 한 행에 넣지 않고, 구간별로 나눈 텍스트를 `content_ai_searchchunk` 행으로 저장합니다. 예를 들면 아래처럼요.

| id | 청크 text |
| --- | --- |
| 1 | postgres 인덱스로 검색 속도를 높인다 |
| 3 | postgres와 pgvector를 같이 쓴다 |

위 두 청크만 보면 `postgres` 토큰은 역인덱스를 통해 행 1, 3에 연결되게 됩니다.
