...
# 테스트는 어떻게?
LLM API로 외부 모델을 가져와서 이걸 통해 결과를 도출하는 것도 내부 로직을 알 수 없는데, 여기서 또 프롬프트로 전처리를 하는 작업을 추가하게 된다면 작은 오차가 더 커질 걸로 보였습니다.

이는 서비스로 간다면 이 작은 오차가 여러 개의 처리를 밟으면서 눈덩이처럼 커지겠죠. 이걸 잡는 작업이 가장 핵심으로 생각됩니다.
