...
위 프로토콜 다이어그램의 (C) Authorization Grant 이전에 앱을 사전 등록하는 작업이며, 초기 설정 단계에서 한번만 수행합니다.
...
추가로, 하나의 서비스가 여러개의 서버 컴포넌트와 브라우저 컴포넌트로 나뉘는 분산 구조일 수도 있습니다.
...
| 항목 | 공통 여부 | 설명 |
| --- | --- | --- |
| (선택) 허용 `response_type` | 공통(정책) | 클라이언트가 인가 요청에서 사용할 수 있도록 권한 서버에 사전 등록하는 응답 타입 정책(예: `code`) |
...
| (선택) `grant_type` | 공통(정책) | 클라이언트가 사용할 수 있는 권한 부여 방식(예: `authorization_code`, `client_credentials`, `refresh_token`) |
| (선택) `scope` | 공통(정책) | 요청 가능한 권한 범위를 사전에 제한, Authorization Server/플랫폼 제공자에서 미리 scope 체계를 설계해둡니다. |
...
public client는 시크릿 기반 인증 대신 PKCE 같은 보완 메커니즘을 사용합니다.
- **※PKCE(Proof Key for Code Exchange)**: OAuth 2.0 및 OpenID Connect 프로토콜에서 인가 코드(Authorization Code) 탈취 공격을 방지하기 위해 설계된 보안 기술
...
실무에서는 보통 **Authorization Code Grant(4.1)** 를 기준으로 읽으면 흐름이 가장 명확합니다.
...
## Refresh Token
리프레시 토큰으로 새 액세스 토큰을 재발급받는 방법을 정의합니다.
...
실무에서 먼저 점검할 항목만 간단히 정리하면 아래와 같습니다.

- `redirect_uri`는 등록값과 정확히 일치하도록 검증합니다. (오픈 리다이렉트/코드 탈취 방지)
- `state`를 사용하고 콜백에서 반드시 검증해 CSRF를 완화합니다.
  - `state`를 클라이언트가 인가 데이터를 보낼 때 `state`값과 콜백으로 돌아온 `state` 값을 비교해서 일치로 판단하므로 CSRF 문제 완화
- public client(SPA/네이티브)는 `authorization_code + PKCE`를 기본으로 사용합니다.
- `Access Token/Refresh Token`은 HTTPS 전송, 안전 저장, 짧은 수명, 최소 scope 원칙을 적용합니다.
- `Refresh Token`은 회전(rotation) 및 의심 시 폐기(revocation) 정책을 운영합니다.
- `confidential client`는 적절한 클라이언트 인증(`client_secret_basic`, 필요시 `private_key_jwt`/`mTLS`)을 사용합니다.
  - 여기말고도 `JWT`는 `OAUTH`에서 다양하게 사용하게 쓰입니다, 액세스 토큰, ID 토큰, client_assertion 등등
  - **※JWT(JSON Web Token)**: JSON 객체를 사용하여 당사자 간에 정보를 안전하게 전송하는 URL-safe한 웹 토큰 표준