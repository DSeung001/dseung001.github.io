...
# 서론
이미 HTTP/1.1로 업데이트되면서 많이 바뀌었지만, 여러 문제를 해결하고자 HTTP/2에서는 프레임, 멀티플렉싱 등이 추가되었죠.
...
HTTP/2는 이 부분을 스트림 우선순위와 스트림 멀티플렉싱 등의 추가로 어느 정도 해소했지만, HTTP/2도 TCP 하나를 공유하므로 HOL은 발생합니다. 그래서 TCP 쪽 HOL 이야기는 HTTP/3로 이어지죠.

HTTP/1.1의 헤더 필드는 중복도 있고 불필요한 데이터도 많아서 불필요한 네트워크 트래픽이 발생했습니다.
거기에 새 TCP 연결은 혼잡 제어나 초기 왕복 등 비용이 있어서, 연결을 자주 새로 여는 방식은 부담이 될 수 있었죠(혼잡 윈도우 자체는 TCP 쪽 동작이고 HTTP/2가 직접 바꾸는 값은 아님)
...
HTTP/2에서 TCP(실무에서는 대개 TLS) 위로 오가는 바이트를 고정 형식의 프레임으로 나누고, 그 프레임이 어느 스트림에 속하는지를 규정하는 레이어입니다. 메서드·상태 코드·헤더 필드 이름 같은 HTTP 의미(semantics) 자체는 RFC 9110 등에서 정의되고, HTTP/2는 “그 메시지를 어떤 순서로, 어떤 단위로 실어 보낼지”를 정의하는 프로토콜이라고 보시면 됩니다.
...
요청과 응답에서는 항상 HEADERS(HPACK)가 있고, DATA는 본문이 있을 경우 같은 스트림에서 이어집니다. 옵션인 거죠.
```mermaid
flowchart TB
