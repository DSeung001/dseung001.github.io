...
효과적안 코어 수랑 배치 사이즈를 제 맥북 m1 air 기준으로 

# 4. 벡터 검색 엔진 구축 

RAG 시스템 구현을 위해 텍스트 쿼리와 프레임 이미지를 같은 CLIP 임베딩 공간에 올려 두고 사용자가 입력한 쿼리 벡터와 가까운 프레임 벡터를 찾는 의미(시맨틱) 검색을 만들어야 합니다, 이때 필요한 건 고차원 벡터를 빠르게 근접 탐색(`ANN`) 해 주는 저장소가 필요하며 **Qdrant**은 그 중 하나입니다.

## Qdrant 맛보기
Qdrant는 벡터를 컬렉션에 적재하고, 거리(코사인, 유클리드 등) 기준으로 가까운 값을 찾아주는 벡터 DB 입니다.
수백만 개의 고차원 벡터 값 중에서 가장 가까운 벡터를 찾는 ANN 근사 최근접를 사용하빈다
- **ANN 근사 최근접**: 데이터 세트에서 주어진 쿼리 요소와 매우 가까운 데이터 요소를 찾는 알고리즘이지만 반드시 절대적으로 가장 가까운 것을 찾는 것은 아닙니다, NN 알고리즘은 모든 데이터를 철저히 검색하여 완벽한 일치 항목을 찾는 반면, ANN 알고리즘은 충분히 가까운 일치 항목을 찾습니다.
