...
```
결국에 나는 어떻게 효율적으로 서비싱하냐에 관점을 두자
```

# 2. 대용량 비동기 전처리 파이프라인
# 3. 하이브리드 검색 엔진 구축 (Qdrant + Elastic Search)
# 4. 대트래픽 대비 API 설계 및 최적화 (DRF & Caching)
# 5. RAG 및 LLM 기반 사용자 경험 (UX/UI 연동)
# 6. 모니터링 및 부하 테스트 (Sentry & Datadog)