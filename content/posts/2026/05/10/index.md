...
이렇게 가져온 LLM이 담당할 로직은 사용자가 입력한 텍스트들을 분석하여 결과적으로 벡터와 매칭할 수 있는 형태로 가공하는 겁니다.
...
하나로도 충분히 제어가 되지만 사용자와 한국어로 의사소통하는 부분과 `CLIP` 모델이 이해하기 쉽게 하는 부분을 나눠서 정확도를 높이고자 했습니다.
...
한글을 영어로 변경 → CLIP 인코딩 → 벡터 검색 → 비슷한 시간은 묶기 → 점수와 개수로 걸러내기 → 반환 형태로 가공
