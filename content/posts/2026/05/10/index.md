...
예를 들어 사용자가 내가 좋아하는 "A" 캐릭터가 "B"와 학교 앞에서 화해했던 장면이라고 검색했을 때 LLM을 통해서 벡터로 검색할 수 있게 바꿔야겠죠. 인물은 2명이고, 장소는 학교 앞으로 해서 이 값들을 벡터에서 비슷한 장면을 찾아야 합니다.
후에는 장르, 애니메이션 시리즈, 화수, 캐릭터 이름 등으로 좀 더 정확한 검색 시스템을 구현해 볼 수 있겠는데 이는 `Elasticsearch`로 나중에 추가해 봅시다.
