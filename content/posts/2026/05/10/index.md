...
# 6. RAG로 확장하기
## 검색 결과를 LLM 컨텍스트로 전달하기
## 프레임, 자막, 메타데이터 기반 답변 생성

`CLIP`과 벡터 DB만 사용하면 사용자의 자연어 쿼리에 가까운 장면을 찾을 수는 있지만, 왜 그 장면이 선택되었는지 설명하거나 여러 후보를 묶어 사용자에게 자연스럽게 답변하기는 어렵습니다.
이 지점에서 `RAG(Retrieval-Augmented Generation)` 구조를 붙일 수 있습니다.

여기서 검색은 장면 후보를 찾아오는 역할을 하고, 생성은 검색된 프레임, 타임스탬프, 자막, 장면 설명, 유사도 점수 등을 근거로 LLM이 답변을 만들어내는 역할을 합니다.
즉 벡터화된 정보를 저장하는 것 자체가 생성은 아니며, 저장된 정보를 검색한 뒤 그 결과를 바탕으로 사용자가 이해하기 쉬운 설명을 만드는 단계가 생성입니다.

전체 흐름은 다음처럼 구성할 수 있습니다.

```text
사용자 질문
→ 텍스트 쿼리 임베딩
→ 벡터 DB에서 관련 프레임/장면 검색
→ 검색 결과와 메타데이터를 LLM 프롬프트에 주입
→ LLM이 근거 기반 답변 생성
```

예를 들어 사용자가 "주인공이 침대 위에서 울고 있는 장면 찾아줘"라고 요청했다면, 검색 단계에서는 관련 프레임과 장면 정보를 가져옵니다.

```json
{
  "episode": 1,
  "timestamp": "00:12:31",
  "visual_query_score": 0.2711,
  "caption": "엘프가 침대 위에서 울고 있고, 일행이 주변에 서 있다."
}
```

이 결과를 LLM에 전달하면 단순히 프레임 번호만 반환하는 대신 다음처럼 답변을 생성할 수 있습니다.

```text
1화 12분 31초 부근의 장면이 가장 적합합니다.
침대 위에서 울고 있는 인물이 중심에 있고, 주변 인물들이 이를 바라보는 구도라서 요청한 장면과 잘 맞습니다.
```

따라서 이 프로젝트의 1차 목표는 `CLIP` 기반 멀티모달 검색 엔진을 구성하는 것이고, 이후 RAG를 붙이면 검색된 장면을 설명하고 요약해주는 인터페이스로 확장할 수 있습니다.
