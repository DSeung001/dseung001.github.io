...
## 실습
### 허깅페이스로 로컬에서 간단히 돌리기
트래픽이 높은 서비스에서는 모델 인퍼런스를 동기 API로 처리하면 장애의 원인이 될 수 있기에, `Celery Worker`나 `AWS ECS` 기반의 독립된 추론 인스턴스로 분리해 두는 설계를 염두에 두고 합니다.
- **Celery Worker**: 비동기 작업을 처리하기 위한 분산 작업 큐 시스템으로, 즉 메인 애플리케이션과 별도로 백그라운드에서 병렬로 처리하는 프로세스 구조.
- **AWS ECS 기반의 독립된 추론 인스턴스**: 머신러닝 모델(AI)이 예측(Inference)을 수행하는 전용 도커 컨테이너를 `Amazon Elastic Container Service(ECS)`를 통해 운영하는 것을 의미함.

여기서는 허깅페이스로 모델을 세팅한 뒤, 이 모델의 기능을 “이미지·텍스트 입력을 받아 고정 차원의 Float 배열(`Vector`)로 바꿔 반환하는 블랙박스 함수”로 정의하고, 함수에 걸리는 시간과 모델별 결과를 비교하며, 텍스트와 이미지의 유사도 측정까지 확인해 봅시다.
