...
AI 모델을 활용해서 **"상황, 대사, 분위기"** 등 자연어 검색으로 애니메이션의 특정 장면을 찾아내는 멀티모달 시맨틱 검색 엔진을 만들어 보려고합니다. 그 전에 앞서 멀티모달 임베딩의 원리와 `CLIP/Whisper` 등에 대해 알아보려 합니다.
...
### Whisper를 적용해보고, Numpy로 DB 없이 검색기 만들기
이제는 좀 더 서비스에 가까운 코드를 만들어봅시다. 지금까지는 `CLIP` 모델로 이미지 프레임과 텍스트 쿼리의 유사도를 비교했지만, 다음 단계에서는 `Whisper`로 오디오를 전사하고 타임스탬프를 붙여 대사 정보까지 함께 활용해보겠습니다. 즉 이미지뿐 아니라 대사 맥락까지 고려해 내가 입력한 문장과 가장 가까운 프레임을 찾는 간단한 검색기를 `Numpy`로 만들어봅시다.
