...
두 스레드가 특정 지점에 둘 다 도달하기 전에는 누구도 그 지점을 빠져나가지 못하게 하는 `rendezvous`도 세마포어로 구현할 수 있습니다.
...
코드 흐름에 `barrier`를 두고, 모든 스레드가 특정 작업을 끝내기 전에는 다음 작업을 진행하지 않도록 만들 수도 있습니다. 아래처럼 세마포어와 카운터를 조합해, N개 스레드가 모두 도착해야 다음 단계로 넘어가게 할 수 있죠.
