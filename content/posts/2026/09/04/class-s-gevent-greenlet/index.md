...
카탈로그가 커지면 사이트맵을 인덱스로 나누는 쪽이 프로토콜 특징에 맞습니다. 하지만 아직 이 프로젝트의 콘텐츠는 많지 않고, 발행 편의성으로 보면 지금 구조가 좋아 보였습니다.
나중에는 업로드 시 사이트맵이 업데이트되는 파이프라인을 고려해 볼 수도 있겠지만, 지금 구조에서는 과해 보이네요.